...
class FTPClient:
    ...
    def upload(self):
        ...
        try:
            ...
            # Try to extract a number from the input (handles "nr 1", "number 1", etc.)
            import re
            number_match = re.search(r'\d+', choice)
            if number_match:
                try:
                    file_index = int(number_match.group()) - 1
                ...
                # No number found, treat as filename
                filename = choice
        ...
        if response['status'] == 'error':
            ...
            if content:
                ...
                try:
                    with open(filepath, 'w') as f:
                        f.write(content)
                    print(f"✓ File saved to {LOCAL_FILES_DIR}/{filename}")
                except Exception as e:
    ...
    def see_file_operation_history(self):
        ...
        filename = input("Enter filename to see history: ").strip()
        if not filename:
            print("✗ Invalid filename")
            return
